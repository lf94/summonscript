...
const { libfive_mesh_delete, libfive_tree_render_mesh } = require("../koffi/libfive");
const { Region3 } = require("./math");

const Viewer = {
    upload: (sdfFn, boundingBox, currentResolution, targetResolution, n = 0) => {
      const p = new Promise((resolve, reject) => {
        console.log("Meshing");
        const meshMemory = libfive_tree_render_mesh(sdfFn.value, Region3(boundingBox[0], boundingBox[1]), currentResolution);
        console.log("Meshed");
      ...
      return p.then(({ cur, next }) => {
        if (cur >= next) return;
        return Viewer.upload(sdfFn, boundingBox, cur * 2, next, n+1);
      });
  }
};
